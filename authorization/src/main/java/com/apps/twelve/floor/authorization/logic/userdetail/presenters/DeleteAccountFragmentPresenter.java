...
@InjectViewState public class DeleteAccountFragmentPresenter
    extends BasePresenter<IDeleteAccountFragment> {
  ...
  public void deleteAccount1(String password) {

    Subscription subscription =
        checkToken(mDataManager.deleteUserProfile1(password)).concatMap(response -> {
          if (response.code() == RESPONSE_TOKEN_EXPIRED) {
            return checkToken(mDataManager.deleteUserProfile1(password));
          }
  ...
  public void verifyCode(String verifyCode) {
    Subscription subscription =
        checkToken(mDataManager.deleteUserProfile2(verifyCode)).concatMap(response -> {
          if (response.code() == RESPONSE_TOKEN_EXPIRED) {
            return checkToken(mDataManager.deleteUserProfile2(verifyCode));
          }
          return Observable.just(response);
        }).compose(ThreadSchedulers.applySchedulers()).doOnNext(response -> {
          if (response.isSuccessful()) {
  ...
  private void handleError(ResponseBody errorBody) {
    ...
    if (content != null) {
      if (content.getPasswordError() != null && !TextUtils.isEmpty(
        ...
        getViewState().showError(content.getPasswordError().get(0));
      }
      if (content.getError() != null && !TextUtils.isEmpty(content.getError().get(0))) {
        getViewState().showError(content.getError().get(0));
      }
    }
  }
